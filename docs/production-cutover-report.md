...
1. **MFA y configuración de OTP** — corregidos en el `config.toml` y confirmado "up to date" contra remoto, pero solo la protección de `enable_confirmations` se verificó con una prueba funcional independiente. Recomendado: confirmar manualmente en el dashboard de Supabase.
...
6. **Hotmart**: cero credenciales conectadas, tal como se instruyó.
...
Las únicas verificaciones no completadas (login Founder por contraseña específicamente, MFA/OTP sin prueba funcional independiente) son huecos de cobertura de un método de prueba puntual, no evidencia de que algo esté roto — están documentadas como riesgos residuales de bajo impacto, no como bloqueantes.
