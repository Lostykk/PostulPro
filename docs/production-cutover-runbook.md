...
**No programar el corte todavía**, pero el margen se redujo a un solo gate real: con A.1 (IA end-to-end) y A.7 (billing Test Mode end-to-end) ya en ✅, lo único que bloquea hoy es una acción humana no técnica — ver `docs/production-go-no-go.md`, sección "Próximo gate único": alguien con acceso a la organización del Supabase anterior corre un conteo de usuarios reales para poder elegir entre los tres escenarios de migración. Google OAuth roto queda como nota secundaria a decidir, no como bloqueante. Ninguno requiere tocar producción para resolverse.
