...
sugerencia de calendario semanal (Lunes a Domingo) indicando qué canal publicar cada día y por qué`;
  ...
  async function copyChannel(i: number) {
    const s = sections[i];
    if (!s) return;
    const text = s.fields.subject ? `${s.fields.subject}\n\n${s.body}${s.fields.cta ? `\n\n${s.fields.cta}` : ""}` : s.body;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">3 créditos · LinkedIn, X, Instagram, Facebook, YouTube + calendario</p>
      </header>

      <div className="grid lg:grid-cols-[380px_1fr] gap-6">
        <aside className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-4 h-fit">
          <Field label="Tema">
            <input className="input" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Ej: Lanzamiento de producto" />
          </Field>
          <Field label="Objetivo">
            <Select value={objective} onChange={setObjective} options={OBJECTIVES} />
          </Field>
          <Field label="Industria">
            <input className="input" value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="Ej: SaaS B2B" />
          </Field>
          <Field label="Tono">
                  ...
                  <div className="space-y-4">
                    ...
                    {sections[activeTab].fields.cta && (
                      <div>
                        ...
                        <div className="text-sm text-violet-300">{sections[activeTab].fields.cta}</div>
                      </div>
                    )}
                  </div>
                )}
