...
import type { StuckStepsReconciliationSummary } from "@/lib/ai/reconcile-stuck-steps.server";

// Internal endpoint dispatching to the "reconcile-stuck-steps" Nitro Task —
// sibling to routes/api/internal/reconcile-stuck-projects.ts, same secret
// (RECONCILE_SECRET, already configured on the preview Worker). See
...
async function handlePost({ request }: { request: Request }) {
  ...
  try {
    const { result } = await runTask("reconcile-stuck-steps", {
      payload: { batchLimit, triggerSource: "http" },
    });
    const summary = result as StuckStepsReconciliationSummary;
    if (!summary.ok) {
...
export const Route = createFileRoute("/api/internal/reconcile-stuck-steps")({
  server: {
    handlers: {
      POST: handlePost,
